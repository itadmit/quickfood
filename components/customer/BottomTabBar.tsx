...
import { IcoHome, IcoBag, IcoCart, IcoUser } from "@/components/shared/Icons";
...
export function BottomTabBar({ tenantSlug }: { tenantSlug: string }) {
  ...
  const tabs = [
    ...
    { href: `/${tenantSlug}/menu`, label: "תפריט", Icon: IcoBag },
    { href: `/${tenantSlug}/cart`, label: "סל", Icon: IcoCart, badge: itemCount > 0 ? itemCount : undefined },
    { href: `/${tenantSlug}/profile`, label: "אזור אישי", Icon: IcoUser },
  ];

  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 max-w-md mx-auto bg-white/95 backdrop-blur border-t border-qf-line">
