...
export class GrowProvider extends BasePaymentProvider {
  ...
  async initiatePayment(req: InitiatePaymentRequest): Promise<InitiatePaymentResponse> {
    ...
    try {
      ...
      // Grow's SDK-wallet method filter is `transactionTypes[]` (PLURAL) where
      // the INDEX is a fixed method slot and the value is that method's code:
      ...
      // every other method and OMIT slot [4]. QuickShop-proven caveat: Grow
      // ignores the ENTIRE whitelist when it lists a method the page isn't
      // set up for (bank transfer would reappear), so the Apple Pay slot is
      // sent only for tenants whose Grow page has it enabled - flip the
      // toggle in dashboard settings/payments after Grow confirms the page.
      // The domain-association file itself is platform-wide (one Grow blob
      // for every storefront domain; Grow approves domains on their side).
      ...
      if (!bankTransferEnabled) {
        ...
        if (this.config!.settings?.applePayEnabled === true) {
  ...
  validateWebhook(_body: unknown, headers: Record<string, string>): WebhookValidationResult {
    ...
    // The IP allowlist is the ONLY guard on this callback - Grow sends no HMAC
    // and the rest of the handler matches by a guessable order number + amount,
    // so without the IP check anyone could POST a forged "paid" callback and
    // mark an unpaid order as paid. Keep it strict for live. (Stranded legit
    // callbacks from a rotated Grow IP are recovered manually + by the planned
    // central Billing-Hub webhook; we do NOT trade payment integrity for it.)
    if (!ip) {
      if (strictlyEnforce) {
        this.logError("Grow callback rejected - no determinable source IP", {});
    ...
    if (!allowed.has(ip)) {
      if (strictlyEnforce) {
        this.logError("Grow callback rejected - IP not in live whitelist", { ip });
