...
const PROMPT = `אתה ממיר תפריט אוכל של מסעדה ישראלית (בדרך כלל בעברית) לפורמט מובנה.
קיבלת קובץ תפריט (PDF או תמונה). חלץ ממנו את כל המנות בדיוק כפי שכתוב.

חשוב מאוד — אל תמציא:
- קרא אך ורק את מה שמופיע בפועל בתפריט. אל תמציא שמות מנות, תוספות, קטגוריות או מילים שלא כתובות.
- אלה שמות של מאכלים ומשקאות בעברית — ודא שכל שם הוא מילה אמיתית והגיונית בתפריט אוכל. אם טקסט מטושטש או לא ברור, רשום רק את החלק הקריא, ואל תנחש מילים.
- אל תמציא מחירים — אם מחיר לא מופיע ליד פריט, השתמש ב-0.
...
- אל תכלול תמונות. שמור על השפה המקורית של התפריט (עברית, ולעיתים אנגלית/ערבית).
