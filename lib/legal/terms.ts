...
export function buildDefaultTerms(ctx: TermsContext): string {
  ...
  return [
    ...
    `האתר מופעל באמצעות מערכת QuickFood, המספקת ל${name} תשתית טכנולוגית בלבד להצגת התפריט, קליטת הזמנות, עדכוני סטטוס והתראות. QuickFood אינה צד לעסקת הרכישה בין הלקוח לבין ${name}, ואינה אחראית למוצרים, להכנתם, לאיכותם, לאספקתם או למדיניות העסק. האחריות המלאה לעסקה ולמוצרים חלה על ${name} בלבד.`,
    ``,
    `## הגבלת גיל`,
    ...
    deliveryClause,
    ``,
    `## עדכוני סטטוס והתראות`,
    `סטטוס ההזמנה המוצג באתר מתבסס על עדכוני ${name} ועל תהליכים אוטומטיים, וזמני ההכנה והאספקה הם הערכה בלבד; ייתכנו פערים או עיכובים. ${name} עשוי לשלוח עדכונים בנוגע להזמנה באמצעות מסרון (SMS), WhatsApp, דוא״ל או התראות דפדפן, בכפוף לפרטים ולהרשאות שמסרת. מסירת ההתראות תלויה ברשת, במכשיר, בדפדפן ובהרשאות, ואינה מובטחת באופן מלא או מיידי - מומלץ לעקוב אחר ההזמנה גם דרך מסך סטטוס ההזמנה באתר. ניתן להפסיק קבלת התראות דרך הגדרות הדפדפן/המכשיר או בפנייה לעסק.`,
    ``,
    ...
    ``,
    `## שימוש אסור`,
    `אין לעשות שימוש באתר לכל מטרה בלתי חוקית; אין לנסות לחדור למערכת, לעקוף אמצעי אבטחה, לבצע הנדסה לאחור (Reverse Engineering) או שימוש אוטומטי מזיק; ואין להעלות או לשלוח תכנים פוגעניים, מטעים או מטרידים. ${name} ו-QuickFood רשאים לחסום גישה לאתר במקרה של הפרת תנאים או שימוש בלתי סביר.`,
    ``,
    `## זמינות המערכת ותחזוקה`,
    `האתר ניתן לשימוש כפי שהוא ("AS IS"). אין התחייבות לזמינות מלאה או לפעילות רציפה וללא תקלות, וייתכנו הפסקות לצורך תחזוקה מתוכננת או עקב נסיבות חריגות. ${name} ו-QuickFood לא יישאו באחריות לנזק שייגרם עקב חוסר זמינות זמני של האתר.`,
    ``,
    `## פרטיות ואבטחת מידע`,
    `הפרטים האישיים שנמסרים בעת ביצוע ההזמנה (שם, טלפון, כתובת דוא״ל וכתובת למשלוח) נאספים לצורך ביצוע ההזמנה, אספקתה ויצירת קשר עם הלקוח בלבד, ואינם נמסרים לצדדים שלישיים אלא לצורך השלמת ההזמנה (כגון ספק סליקה ושירות שליחויות) או כנדרש על פי דין. כן נאסף מידע טכני, כגון פרטי מכשיר ודפדפן, יומני מערכת ומזהי התראות, לצורך תפעול האתר ואבטחתו. המידע נשמר באמצעים מקובלים ובאמצעי אבטחה סבירים כדי להגן עליו מפני גישה או שימוש לא מורשים, אך ללא התחייבות לחסינות מוחלטת; גיבויים ויומני מערכת עשויים להישמר לתקופה מוגבלת גם לאחר מחיקה תפעולית. דיוור פרסומי יישלח רק ללקוחות שנתנו לכך את הסכמתם, וניתן להסיר את ההסכמה בכל עת.`,
    ``,
    `## שינוי התקנון`,
    `${name} רשאי לעדכן תקנון זה מעת לעת. הנוסח המעודכן יחול ממועד פרסומו באתר, והמשך השימוש באתר לאחר העדכון מהווה הסכמה לתנאים המעודכנים.`,
    ``,
