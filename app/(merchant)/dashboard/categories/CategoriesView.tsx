...
export function CategoriesView({ categories }: { categories: CategoryRow[] }) {
  ...
  async function save() {
    ...
    setEditing(null);
    pushToast("ok", isCreate ? "הקטגוריה נוצרה" : "הקטגוריה עודכנה");
    router.refresh();
  }
  ...
  const totalItems = orderedCategories.reduce((sum, c) => sum + c.itemCount, 0);
  ...
  return (
    <div className="space-y-4 lg:space-y-5">
      <PageHeader
        chip="קטלוג"
        title="קטגוריות"
        subtitle={`${orderedCategories.length} קטגוריות · ${totalItems} פריטים בסך הכל`}
        actions={
          <button
            type="button"
            onClick={openNew}
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-black text-[#F8CB1E] border-2 border-black font-bold text-sm shadow-[0_2px_0_#000] hover:bg-black/90"
          >
      ...
      <div className="bg-white rounded-2xl border border-qf-line-dash overflow-hidden">
        {orderedCategories.length === 0 ? (
          <div className="p-10 text-center text-sm text-qf-mute space-y-3">
            <div>אין עדיין קטגוריות.</div>
            <button
              type="button"
              onClick={openNew}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-black text-[#F8CB1E] border-2 border-black font-bold text-sm shadow-[0_2px_0_#000] hover:bg-black/90"
          ...
          >
            {(c, _i, drag) => {
              ...
              return (
                <div
                  className={cn(
                    "flex items-center gap-3 px-3 lg:px-4 py-3 border-b border-qf-line-soft last:border-b-0 bg-white",
                    drag.isDragging && "opacity-50",
                  )}
                >
                  <span
                    {...drag.handleProps}
                    className="grid place-items-center w-6 h-9 rounded-md hover:bg-qf-line-soft cursor-grab active:cursor-grabbing shrink-0 text-qf-mute"
                  >
                    <DragHandle />
                  </span>
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-qf-mute mt-0.5 flex items-center gap-2 flex-wrap">
                      <span>{c.itemCount} פריטים</span>
                      {c.upsellInCart && (
                        <>
                          <span>·</span>
                          <span>אפסייל בעגלה</span>
                        </>
                      )}
                      {c.upsellBeforeCheckout && (
                        <>
                          <span>·</span>
                          <span>תזכורת לפני סגירה</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-9 h-9 rounded-lg hover:bg-qf-tomato-soft grid place-items-center"
                    aria-label="מחק"
                    title="מחק"
                  >
                    <IcoTrash c="#c2421f" s={16} />
                  </button>
                </div>
              );
            }}
...
function DraftModal({
  ...
  return (
    <Modal
      ...
      onClose={onCancel}
      size="lg"
      ariaLabel={draft.id ? "עריכת קטגוריה" : "קטגוריה חדשה"}
    >
      <header className="sticky top-0 z-10 bg-white flex items-center justify-between px-5 py-4 border-b border-qf-line">
        <h2 className="font-semibold text-lg">
          {draft.id ? "עריכת קטגוריה" : "קטגוריה חדשה"}
        </h2>
        <button
          type="button"
          onClick={onCancel}
